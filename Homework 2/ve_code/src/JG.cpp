...
long double ABS_ERROR(vector<Double>& t1, vector<Double>& t2) {
	long double error = 0.0;
	if (t1.size() != t2.size()) {
		return 0.0;
	}
	for (int i = 0; i < t1.size(); i++) {
		error += fabs(t1[i] - t2[i]);
	}
	return error;
}
struct EDGE_RESIDUAL_HELPER {
	JGEdge* edge;
	bool m;
	long double error;
};
struct EDGE_RESIDUAL_HELPER_COMPARATOR {
	bool operator()(EDGE_RESIDUAL_HELPER const & a,
			EDGE_RESIDUAL_HELPER const & b) {
...
JG::JG(GM& gm, int i_bound__, int num_iterations__, vector<int>& order,
	...
	for (int i = 0; i < gm.functions.size(); i++) {
		...
		for (int j = 0; j < gm.functions[i]->variables().size(); j++) {
			...
			if (mapped_order[gm.functions[i]->variables()[j]->id()]
					< min_size) {
		...
		//cout<<min_size<<endl;
		buckets[min_size].push_back(
				JGHelperFunction(gm.functions[i]->variables(), INVALID_VALUE,
						gm.functions[i]));
		//cout<<"\tAdded\n";
	}
	...
	for (int i = 0; i < order.size(); i++) {
		...
		if (i < (int) order.size() - 1) {
			for (int j = old_nodes; j < nodes.size(); j++) {
				...
				//	continue;
				buckets[min_size].push_back(
						JGHelperFunction(temp_variables, j));
			}
		}
		...
		for (int j = 0; j < nodes.size(); j++) {
			...
			for (int k = 0; k < nodes[curr_node]->edges().size(); k++) {
				if (nodes[curr_node]->edges()[k]->node1()->id()
					...
					assert(
							nodes[curr_node]->edges()[k]->node2()->id()==nodes[curr_node]->id());
...
bool JG::propagate_Residual() {
	...
	// Find all edges and put them in a vector
	set<JGEdge*> edge_set;
	for (int j = 0; j < nodes.size(); j++) {
		for (int k = 0; k < nodes[j]->edges().size(); k++) {
			edge_set.insert(nodes[j]->edges()[k]);
		}
	}
	vector<EDGE_RESIDUAL_HELPER> all_edges(edge_set.size() * 2);
	int j = 0;
	for (set<JGEdge*>::iterator i = edge_set.begin(); i != edge_set.end();
			i++) {
		all_edges[j].edge = *i;
		all_edges[j].m = true;
		all_edges[j].error = 0.0;
		j++;
		all_edges[j].edge = *i;
		all_edges[j].m = false;
		all_edges[j].error = 0.0;
	...
	for (int j = 0; j < all_edges.size(); j++) {
		if (all_edges[j].m)
			all_edges[j].edge->message1();
		else
			all_edges[j].edge->message2();
	}
	for (int i = 1; i < num_iterations_; i++) {
		error = 0.0;
		for (int j = 0; j < all_edges.size(); j++) {
			if (all_edges[j].m) {
				vector<Double> t = all_edges[j].edge->message1().table();
				all_edges[j].edge->sendMessage1to2();
				all_edges[j].error = ABS_ERROR(t,
						all_edges[j].edge->message1().table());
				error += all_edges[j].error;
			} else {
				vector<Double> t = all_edges[j].edge->message2().table();
				all_edges[j].edge->sendMessage2to1();
				all_edges[j].error = ABS_ERROR(t,
						all_edges[j].edge->message2().table());
				error += all_edges[j].error;
			}
		}
		cerr << "Iteration " << i << " Done\n";
		cerr << "Error =" << error << endl;
		//if (error < 0.0001 && i>=10)
		//	break;
		if (convergence_test())
			break;
		sort(all_edges.begin(), all_edges.end(),
				EDGE_RESIDUAL_HELPER_COMPARATOR());
	}
	updateMarginals();
	for (int i = 0; i < nodes.size(); i++) {
		nodes[i]->updateVariables();
...
void JG::clear() {
	...
	for (set<JGEdge*>::iterator i = all_edges.begin(); i != all_edges.end();
			i++) {
		if ((*i) == NULL)
			continue;
		delete (*i);
