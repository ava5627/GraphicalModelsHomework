...
JT::JT(vector<Variable*>& variables_, vector<Function*>& functions, vector<set<int> >& clusters, vector<int>& order): variables(variables_)
{
	...
	for(int i=0;i<clusters.size();i++)
	{
		...
		if(curr_cluster.find(order[i])==curr_cluster.end()){
			cerr<<order[i]<<" not found in cluster "<<i<<", Cluster: ";
		...
		{
			edges.push_back(new JGEdgeSS(dynamic_cast<JGNodeSS*> (nodes[i]),dynamic_cast<JGNodeSS*> (nodes[pos])));
			set<int> temp;
