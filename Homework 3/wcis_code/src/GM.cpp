...
bool GM::verifyWCutset(vector<int> &order, vector<int> &wcutset, int w)
{
    ...
    {
        for(int j=0;j<wcutset.size();j++)
            clusters[i].erase(wcutset[j]);
        if ((int)clusters[i].size()>max_cluster_size)
            max_cluster_size=clusters[i].size();
        if ((int)clusters[i].size()>(w+1)){
            return false;
        }
    }
    cout<<"Verified W-cutset,  w = "<<w<<", width of ordering = "<<max_cluster_size-1<<endl;
    return true;
}
...
void GM::getWCutset(vector<int> &order, vector<set<int>> &clusters, vector<int> &wcutset, int w)
{
    wcutset=vector<int>();
    vector<vector<int> > var2clusters (variables.size());
    vector<int> var2estimate(variables.size(),0);
    vector<int> cluster2estimate(clusters.size(),0);
    set<int> faulting_clusters;
    vector<bool> processed(variables.size());
    for(int i=0;i<clusters.size();i++)
    {
        ...
        cluster2estimate[i]=clusters[i].size();
        if(cluster2estimate[i]>(w+1))
        {
            faulting_clusters.insert(i);
            for(set<int>::iterator j=clusters[i].begin();j!=clusters[i].end();j++)
            {
    ...
    while(!faulting_clusters.empty())
    {
        if(wcutset.size()==order.size())
            break;
        int max_value=clusters.size();
        int max_id=-1;
        bool first=true;
        ...
        for(int i=0;i<variables.size();i++)
        {
            ...
            {
                first=false;
                max_value=var2estimate[i];
                max_id=i;
            }
            else
            {
                if(max_value <var2estimate[i])
                {
                    max_value=var2estimate[i];
                    max_id=i;
                }
        ...
        processed[max_id]=true;
        wcutset.push_back(max_id);


        //Update cluster2estimate
        for(int i=0;i<var2clusters[max_id].size();i++)
        {
            int cluster_id=var2clusters[max_id][i];
            if(faulting_clusters.find(cluster_id)!=faulting_clusters.end())
            {
                cluster2estimate[cluster_id]--;
                if(cluster2estimate[cluster_id]<=(w+1))
                {
                    faulting_clusters.erase(cluster_id);
                    for(set<int>::iterator j=clusters[cluster_id].begin();j!=clusters[cluster_id].end();j++)
                    {
...
void GM::outputSample(ostream& out)
{
	for(int i=0;i<copy_of_variables.size();i++)
	{
		assert(copy_of_variables[i]->value()!=INVALID_VALUE);
		int val=copy_of_variables[i]->value();
		out<<copy_of_variables[i]->mapping[val]<<" ";
	}
	out<<endl;
}
